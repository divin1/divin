...
export async function GET() {
  const posts = await getPosts(PROJECTS_PATH);

  const feed = new Feed({
    title: "Projects",
    description: APP_CONFIG.metadata.description,
    id: APP_CONFIG.domain + "/projects/rss.xml",
    link: APP_CONFIG.domain + "/projects/rss.xml",
    language: "en",
    ...
    generator: "awesome",
    feedLinks: {
      atom: `${APP_CONFIG.domain}/projects/rss.xml`,
    },
    author: {
  ...
  posts.forEach((post) => {
    const url = `${APP_CONFIG.domain}/projects/${post.slug}`;
